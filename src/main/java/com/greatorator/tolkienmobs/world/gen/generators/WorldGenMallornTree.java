...
public class WorldGenMallornTree extends WorldGenAbstractTree {
    ...
    public WorldGenMallornTree(boolean notify)
    {
        super(notify);
    }
    ...
    void generateLeafNodeList()
    {
        ...
        this.foliageCoords = Lists.<WorldGenMallornTree.FoliageCoordinates>newArrayList();
        this.foliageCoords.add(new WorldGenMallornTree.FoliageCoordinates(this.basePos.up(k), j));
            ...
            if (f >= 0.0F)
            {
                for (int l = 0; l < i; ++l)
                {
                    ...
                    if (this.checkBlockLine(blockpos, blockpos1) == -1)
                    {
                        ...
                        {
                            this.foliageCoords.add(new WorldGenMallornTree.FoliageCoordinates(blockpos, blockpos2.getY()));
                        }
                    }
                }
            }
    ...
    void generateLeafNode(BlockPos pos)
    {
        ...
        {
            this.crosSection(pos.up(i), this.leafSize(i), LEAF.withProperty(BlockLeaf.CHECK_DECAY, Boolean.valueOf(false)));
        }
    }
    ...
    private BlockLogs.EnumAxis getLogAxis(BlockPos p_175938_1_, BlockPos p_175938_2_)
    {
        BlockLogs.EnumAxis blocklogs$enumaxis = BlockLogs.EnumAxis.Y;
        ...
        if (k > 0)
        {
            if (i == k)
            {
                blocklogs$enumaxis = BlockLogs.EnumAxis.X;
            }
            else if (j == k)
            {
                blocklogs$enumaxis = BlockLogs.EnumAxis.Z;
            }
        }
    ...
    void generateLeaves()
    {
        for (WorldGenMallornTree.FoliageCoordinates worldgenmallorntree$foliagecoordinates : this.foliageCoords)
        {
            this.generateLeafNode(worldgenmallorntree$foliagecoordinates);
        }
    }
    ...
    void generateLeafNodeBases()
    {
        for (WorldGenMallornTree.FoliageCoordinates worldgenmallorntree$foliagecoordinates : this.foliageCoords)
        {
            int i = worldgenmallorntree$foliagecoordinates.getBranchBase();
            BlockPos blockpos = new BlockPos(this.basePos.getX(), i, this.basePos.getZ());

            if (!blockpos.equals(worldgenmallorntree$foliagecoordinates) && this.leafNodeNeedsBase(i - this.basePos.getY()))
            {
                this.limb(blockpos, worldgenmallorntree$foliagecoordinates, TTMFeatures.LOGS);
            }
        }
    }

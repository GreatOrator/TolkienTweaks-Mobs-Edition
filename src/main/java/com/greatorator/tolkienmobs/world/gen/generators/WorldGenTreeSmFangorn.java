...
public class WorldGenTreeSmFangorn extends WorldGenAbstractTree {
    public WorldGenTreeSmFangorn(boolean p_i45461_1_)
    {
        super(p_i45461_1_);
    }
    ...
    protected boolean canGrowInto(Block blockType) {
        return super.canGrowInto(blockType) || blockType == TTMFeatures.SAPLINGS || blockType == TTMFeatures.LOGS || blockType == TTMFeatures.LEAVES;
    }
    ...
    public boolean generate(World world, Random random, BlockPos pos) {
        ...
        if (y >= 1 && y + treeHeight + 1 <= 256)
        {
            ...
            else //Actually generate tree
            {
                ...
                IBlockState logType = Blocks.LOG2.getDefaultState().withProperty(BlockNewLog.VARIANT, BlockPlanks.EnumType.DARK_OAK).withProperty(BlockLogs.LOG_AXIS, BlockLog.EnumAxis.NONE);
                ...
                if (isSoil && y < 256 - treeHeight - 1)
                {
                    ...
                    for (j2 = 0; j2 < treeHeight; ++j2)
                    {
                        ...
                        {
                            this.setBlockAndNotifyAdequately(world, new BlockPos(trunkX, trunkY, trunkZ), logType);
                            this.setBlockAndNotifyAdequately(world, new BlockPos(trunkX + 1, trunkY, trunkZ), logType);
                            this.setBlockAndNotifyAdequately(world, new BlockPos(trunkX, trunkY, trunkZ + 1), logType);
                            this.setBlockAndNotifyAdequately(world, new BlockPos(trunkX + 1, trunkY, trunkZ + 1), logType);
                            i2 = trunkY;
                        }
                    }

                    //Genthe root
                    this.setBlockAndNotifyAdequately(world, new BlockPos(trunkX - 1, y, trunkZ), logType);
                    this.setBlockAndNotifyAdequately(world, new BlockPos(trunkX + 2, y, trunkZ), logType);
                    this.setBlockAndNotifyAdequately(world, new BlockPos(trunkX + 2, y, trunkZ + 1), logType);
                    this.setBlockAndNotifyAdequately(world, new BlockPos(trunkX - 1, y, trunkZ + 1), logType);

                    this.setBlockAndNotifyAdequately(world, new BlockPos(trunkX, y, trunkZ - 1), logType);
                    this.setBlockAndNotifyAdequately(world, new BlockPos(trunkX + 1, y, trunkZ - 1), logType);
                    this.setBlockAndNotifyAdequately(world, new BlockPos(trunkX + 1, y, trunkZ + 2), logType);
                    this.setBlockAndNotifyAdequately(world, new BlockPos(trunkX, y, trunkZ + 2), logType);
    ...
    private boolean replaceBlockWithWood(World world, int x, int y, int z, EnumFacing direction) {
        ...
        if( block.isReplaceable(world, pos) ||  block.isLeaves(state, world, pos) || block == logType) {
            this.setBlockAndNotifyAdequately(world, pos, logType);
            return true;
        }
        else
            return false;
    ...
    private void replaceAirWithLeaves(World world, int x, int y, int z)
    {
        ...
        IBlockState leafType = Blocks.LEAVES2.getDefaultState().withProperty(BlockNewLeaf.VARIANT, BlockPlanks.EnumType.DARK_OAK).withProperty(BlockLeaves.CHECK_DECAY, Boolean.valueOf(false));
        BlockPos pos = new BlockPos(x, y, z);

        if (world.isAirBlock(pos))
        {
            this.setBlockAndNotifyAdequately(world, pos, leafType);
        }
    }

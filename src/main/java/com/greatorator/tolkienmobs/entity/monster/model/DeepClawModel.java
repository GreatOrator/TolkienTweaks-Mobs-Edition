package com.greatorator.tolkienmobs.entity.monster.model;

import com.greatorator.tolkienmobs.entity.monster.DeepClawEntity;
import com.greatorator.tolkienmobs.entity.monster.render.DeepClawRender;
import net.minecraft.resources.ResourceLocation;
import software.bernie.geckolib3.core.event.predicate.AnimationEvent;
import software.bernie.geckolib3.core.processor.IBone;
import software.bernie.geckolib3.model.AnimatedGeoModel;
import software.bernie.geckolib3.model.provider.data.EntityModelData;

import static com.greatorator.tolkienmobs.TolkienMobs.MODID;

/** DeepClaw - GreatOrator */
@SuppressWarnings({ "unchecked", "removal" })
public class DeepClawModel extends AnimatedGeoModel<DeepClawEntity> {
    @Override
    public ResourceLocation getModelLocation(DeepClawEntity object) {
        return new ResourceLocation(MODID, "geo/monster/deepclaw.geo.json");
    }

    @Override
    public ResourceLocation getTextureLocation(DeepClawEntity object) {
        return DeepClawRender.LOCATION_BY_VARIANT.get(object.getVariant());
    }

    @Override
    public ResourceLocation getAnimationFileLocation(DeepClawEntity animatable) {
        return new ResourceLocation(MODID, "animations/monster/deepclaw.animation.json");
    }

    @Override
    public void setLivingAnimations(DeepClawEntity entity, Integer uniqueID, AnimationEvent customPredicate) {
        super.setLivingAnimations(entity, uniqueID, customPredicate);
        IBone head = this.getAnimationProcessor().getBone("head");

        EntityModelData extraData = (EntityModelData) customPredicate.getExtraDataOfType(EntityModelData.class).get(0);
        if (head != null) {
            head.setRotationX(extraData.headPitch * ((float) Math.PI / 180F));
            head.setRotationY(extraData.netHeadYaw * ((float) Math.PI / 180F));
        }
    }
}
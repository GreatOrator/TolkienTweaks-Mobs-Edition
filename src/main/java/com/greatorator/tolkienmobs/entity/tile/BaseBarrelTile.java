...
public class BaseBarrelTile extends TileBCore implements MenuProvider, IInteractTile, IRSSwitchable {
    public static final ContainerSlotLayout.LayoutFactory<BaseBarrelTile> SLOT_LAYOUT = (player, tile) -> new ContainerSlotLayout().playerMain(player).allTile(tile.itemHandler);
    public TileItemStackHandler itemHandler = new TileItemStackHandler(27);

    public BaseBarrelTile(BlockPos pos, BlockState state) {
        super(TolkienTiles.BARREL_TILE.get(), pos, state);
    ...
    public AbstractContainerMenu createMenu(int windowID, Inventory playerInventory, Player playerEntity) {
        return new ContainerBCTile<>(TolkienContainers.BARREL_BASE_CONTAINER, windowID, playerInventory, this, SLOT_LAYOUT);
    }
}
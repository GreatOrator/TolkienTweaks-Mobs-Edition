...
public class TTMInventoryActions {
    public static boolean transferContainerTank(ITTMBackpackInventory inv, FluidTank tank, int slotIn, @Nullable PlayerEntity player)
    {
        ...
        if(container.isPresent())
        {
            ...
            if(fluidstack.isPresent() && fluidstack.map(FluidStack::getAmount).orElse(0) > 0)
            {
                ...
                if(slotOutStack.isEmpty() || slotOutStack.getItem() == stackOut.getItem())
                {
                    ...
                    inv.setDataChanged(ITTMBackpackInventory.TANKS_DATA);

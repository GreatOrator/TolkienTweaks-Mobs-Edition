...
public class ModelMithrilGolem extends ModelTTM {
    ...
    {
        EntityTMMithrilGolem entitymithrilgolem = (EntityTMMithrilGolem)entitylivingbaseIn;
        int i = entitymithrilgolem.getAttackTimer();

        if (i > 0)
        {
            this.bipedRightArm.rotateAngleX = -2.0F + 1.5F * this.triangleWave((float)i - partialTickTime, 10.0F);
            this.bipedLeftArm.rotateAngleX = -2.0F + 1.5F * this.triangleWave((float)i - partialTickTime, 10.0F);
        }
        else
        {
            int j = entitymithrilgolem.getHoldRoseTick();

            if (j > 0)
            {
                this.bipedRightArm.rotateAngleX = -0.8F + 0.025F * this.triangleWave((float)j, 70.0F);
    ...
    public static void setRenderGolem(boolean renderGolem) {
        ModelMithrilGolem.renderGolem = renderGolem;
    }
